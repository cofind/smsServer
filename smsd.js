...
var queryString = 'select * from smsd.outbox where id = (select min(id) from smsd.outbox)';
 
connection.query(queryString, function(err, rows, fields) {
    if (err) throw err;
 
    for (var i in rows) {
        console.log('SMS ID:', rows[i].ID);
		
            var hex = createFullHeader(createHeader("1"), buildSMS(rows[i].DestinationNumber,'0', rows[i].TextDecoded));
            Logger("[DATA] Sending SMS to number " + rows[i].DestinationNumber, false);
            var buffer = new Buffer(hex, 'hex');
            socket.write(buffer);
            Logger("[PROCESS] -> " + hex, true);
...
var post  = {ID: rows[i].ID};
var query2 = connection.query('delete from outbox where ?', post, function(err, result) {
...
var post  = {ReceivingDateTime: data["time"], Text: 'NA',SenderNumber: data["number"], Coding: data["encoding"],TextDecoded: data["content"],processed: 'false'};
var query = connection.query('INSERT INTO inbox SET ?', post, function(err, result) {
